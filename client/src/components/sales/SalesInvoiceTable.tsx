                    ...
                    <thead className="bg-slate-50">
                        <tr className="border-b border-slate-200">
                            {COLUMNS.map((col) => (
                                <th
                                    key={col.label}
                                    className={`px-4 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-400 text-${col.align}`}
                                >
                                    {col.label}
                                </th>
                            ))}
                    ...
                    <tbody>
                        {invoices.map((inv, idx) => (
                            ...
                            >
                                ...
                                {/* Customer */}
                                <td className="px-4 py-1.5 text-xs font-medium text-slate-800">{inv.customer}</td>

                                {/* Mobile */}
                                <td className="px-4 py-1.5 text-xs text-slate-500">{inv.mobile}</td>

                                {/* Items */}
                                <td className="px-4 py-1.5 text-center text-xs text-slate-700">{inv.items}</td>

                                {/* Qty */}
                                <td className="px-4 py-1.5 text-center text-xs text-slate-700">{inv.qty}</td>

                                {/* Amount */}
                                <td className="px-4 py-1.5 text-xs font-semibold text-slate-800">{inv.amount}</td>
                                ...
                                {/* Salesman */}
                                <td className="px-4 py-1.5 text-xs text-slate-600">{inv.salesman}</td>
